# -------------------------------------------------------------
# Tropical Pacific SST and Global Air Temperature Map
# -------------------------------------------------------------
#
# Create a global correlation map between annual Tropical Pacific
# SST anomalies and annual air temperature anomalies.
#
# Complete the following steps:
#
# 1. Read the monthly air teemperature dataset.
# 2. Calculate annual air temperature anomalies.
# 3. Calculate the Pearson correlation coefficient between the
#    Tropical Pacific SST anomaly time series and annual
#    air temperature anomalies at every grid point.
# 4. Plot the spatial correlation map.
#
# -------------------------------------------------------------
...
# -------------------------------------------------------
# Step 1:
# Read the monthly air temperature dataset and
# store the precipitation variable as "airt".
#
# airt: monthly air temperature data
#
# Hint:
# Use ncdump to identify the variable and coordinate names.
#
# ncdump ../../data/air.2m.mon.mean.nc | less
#

airt_file = repo_dir / "data" / "air.2m.mon.mean.nc"

ds_airt = xr.open_dataset(airt_file)
airt = ds_airt["air"]
...
# Step 2:
# Calculate the annual mean air temperature anomaly
# using calc_seasonal_anom() function.
#

airt_anom = calc_seasonal_anom(airt,window=12,end_month=12)


# Select the analysis period
# Restrict both annual anomaly datasets to the user-defined
# start and end years before further analysis.

sst_anom = sst_anom.sel(year=slice(start_year, end_year))
airt_anom = airt_anom.sel(year=slice(start_year, end_year))
...
airt_for_corr = airt_for_corr.sel(year=common_years)
...
# the Tropical Pacific SST anomaly time series and annual
# air temperature anomalies at every grid point
# using xr.corr() function.
#
# Hint:
# - Use the annual air temperature anomalies (airt_for_corr)
#   as the spatial field.
# - Use the Tropical Pacific SST anomaly time series
#   (tp_sst_for_corr) as the reference series.
# - Compute the correlation along the "year" dimension.
#
# Variable names:
# airt_for_corr    : annual air temperature anomalies
# tp_sst_for_corr : Tropical Pacific SST anomaly time series
# corr_map        : Spatial Pearson correlation coefficients
...
# Tropical Pacific SST time series
...
# Step 4:
# Plot the spatial correlation map using contourf().
# Use longitude, latitude, and the correlation map
# as the three main inputs.


cf = ax1.contourf(
    lon_cyclic,   # Longitude
    corr_map.lat, # Latitude
    corr_cyclic,  # 2-D correlation coefficient field
    levels=levels,
    cmap="RdBu_r",
    extend="both",
...
# Do not cover the correlation field over land
...
ax1.set_title(
    f"Correlation of Global Air Temperature with Tropical Pacific {analysis_label}"
)
